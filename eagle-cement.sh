...
write_server_env() {
    ...
    install -d -o "$SVC_USER" -g "$SVC_USER" -m 755 "$INST_DIR/server"
    {
        ...
        echo
        echo "# Barrier hardware trigger (via ANPR camera AlarmOut relay)"
        echo "BARRIER_CAMERA_HOST=${BARRIER_CAMERA_HOST:-192.168.10.235}"
        echo "BARRIER_CAMERA_PORT=${BARRIER_CAMERA_PORT:-80}"
        echo "BARRIER_CAMERA_USER=${BARRIER_CAMERA_USER:-admin}"
        echo "BARRIER_CAMERA_PASSWORD=${BARRIER_CAMERA_PASSWORD:-admin123}"
        echo "BARRIER_TRIGGER_ENABLED=${BARRIER_TRIGGER_ENABLED:-true}"
        echo "BARRIER_PULSE_DURATION_MS=${BARRIER_PULSE_DURATION_MS:-800}"
    } > "$envfile"
